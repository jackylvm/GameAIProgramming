...
namespace MS.FileRoller
{
    public class DatedFileRoller : BaseFileRoller<DatedFileRoller.DatedFileInfo>
    {
        ...
        protected override DatedFileInfo ExtractFileInfo(Match match)
        {
            ...
            if (hasDate)
            {
                if (DateTime.TryParseExact(dateGroup.Value, "yyyyMMdd", CultureInfo.InvariantCulture,
        ...
        public static string GetDatedFileName(string baseFileName, DateTime date, int index = 0, bool keepExts = false)
        {
