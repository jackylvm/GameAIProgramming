...
namespace MS.Log4Unity
{
    ...
    public class PatternHelper
    {
        ...
        static PatternHelper()
        {
            ...
            RegisterPatternFieldResolver('c', (ctx) => { return ctx.logger.catagory; });
            RegisterPatternFieldResolver('r', (ctx) => { return DateTime.Now.ToLocalTime().ToLongTimeString(); });
            RegisterPatternFieldResolver('p', (ctx) => { return ctx.logType.ToString(); });
            RegisterPatternFieldResolver('m', (ctx) => { return ctx.message; });
            RegisterPatternFieldResolver('d', (ctx) => { return DateTime.Now.ToString(ctx.parameter); });
            RegisterPatternFieldResolver('n', (ctx) => { return "\n"; });
            //start color block
