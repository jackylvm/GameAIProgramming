    ...
    public class FileAppender : LayoutAppender
    {
        ...
        private static byte[] NEW_LINE
        {
            get
            {
                if (_NEW_LINE == null)
                {
                    _NEW_LINE = Encoding.UTF8.GetBytes("\n");
                }
        ...
        public override void OnInitialize(ConfigsReader configs)
        {
            ...
            if (rollTypeStr != null)
            {
                if (!Enum.TryParse<RollType>(rollTypeStr, out rollType))
                {
                    Debug.LogWarning($"bad rollType:{rollTypeStr}");
        ...
        private async void WriteMessageToFileAsync()
        {
            ...
            var deltaTime = DateTime.Now - _lastFlushTime;
